...
describe('calculateBid', () => {
  ...
  it('should calculate bid', async () => {
    const bid1 = await calculateBid({ initialAmount: 1e8 } as any, price1, 0.1, 12, 8);
    expect(bid1).toBe('9000000000000000');

    const bid2 = await calculateBid({ initialAmount: 0.1e8 } as any, price1, 0.2, 12, 8);
    expect(bid2).toBe('800000000000000');

    const bid3 = await calculateBid({ initialAmount: 0.1e8 } as any, price2, 0.1, 12, 8);
    expect(bid3).toBe('889227058938324');
  });

  it('should throw because of last bid is greater', async () => {
    const bid = calculateBid({ initialAmount: 1e8, lastBid: 9_000e12 } as any, price1, 0.1, 12, 8);
